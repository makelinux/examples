...
template <typename T>
requires is_integral_v<T> T constexpr requires_demo(T a) { return a + 1; }
...
template <typename T>
auto constexpr requires_demo(T&& a) requires is_same_v<T, double> { return 2; }
...
template <class T>
concept integral_req_ct = requires(T a)
{
    is_integral_v<T>;
};

template <integral_req_ct T>
constexpr T _inc2(T a) { return a + 1; }
...
template <typename T>
concept compound_requirements = requires(T x)
{
    {
        x + 1
        } -> convertible_to<bool>;
